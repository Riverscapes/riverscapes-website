...
![Data Standards](../../../../src/images/our-work/standards/data-standards-01.png)
...
There is one Business Logic XSD ruleset  that specifies which tags can be used and how to combine them to define the hierarchical tree structure for each project type. Each model has its own business logic XML file.
...
Follow these steps, all of which are underpinned by the FAIR principles,  to apply the riverscapes standards to your data. 

1. Ensure that all the data are in open, non-proprietary formats. This is especially important for geospatial data. We recommend GeoTiffs for rasters and GeoPackages for vector data. ShapeFiles, although a published format are discouraged for numerous reasons.
...
3. Provide an accompanying XML file called project.rs.xml at the root of the folder structure. This XML file has a strict, but helpful structure that helps catalog the individual data files. This XML file must refer to the latest version of the Riverscapes Data Standards published online. The best way to ensure compliance is to refer to the XSD file and then validate your XML file using a tool such as Visual Studio Code. Data with a project.rs.xml file that does not validate cannot be incorporated into the Riverscapes Data Exchange.
